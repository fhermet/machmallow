...
int main(int argc, char** argv) {
    ...
    std::size_t npBase = 0, npAmr = 0, npSub = 0;
    ...
    std::printf("  AMR 2 niv.       : p=%.3f (%zu patches) err %.2f%%\n",
                pAmr, npAmr, 100 * eAmr);
    std::printf("  AMR 2 niv. subcyc: p=%.3f (%zu patches) err %.2f%%\n",
                pSub, npSub, 100 * eSub);
    std::printf("  exact %.3f | AMR vs base %.2f%% (gate 2%%) | "
                "exact gate 5%%\n", pExact, 100 * eBase);

    const bool ok = eAmr < 0.05 && eSub < 0.05 && eBase < 0.02 &&
                    npAmr > 0 && npSub > 0;
    std::printf("%s\n", ok ? "PASS" : "FAIL");
    return ok ? 0 : 1;
}
