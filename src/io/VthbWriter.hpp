...
// Multi-level variant (AmrML / AmrGpuML interface: numLevels(),
// level(l).patches, patchRef(l, p)).
template <class AMR>
inline void writeVthbML(const std::string& base, const AMR& amr) {
    namespace fs = std::filesystem;
    fs::create_directories(base);
    const std::string leaf = fs::path(base).filename().string();
    ...
    for (int l = 1; l < amr.numLevels(); ++l) {
        std::fprintf(f, "<Block level=\"%d\" spacing=\"%g %g 1\">\n", l,
                     double(c.dx) / (1 << l), double(c.dy) / (1 << l));
        const auto& patches = amr.level(l).patches;
        for (std::size_t k = 0; k < patches.size(); ++k) {
            ...
            std::fprintf(f,
                         ...
                         "0 0\" file=\"%s/%s\"/>\n",
                         k, 2 * patches[k].ci0,
                         2 * patches[k].ci0 + nf - 1, 2 * patches[k].cj0,
                         2 * patches[k].cj0 + nf - 1, leaf.c_str(), name);
        }
        std::fprintf(f, "</Block>\n");
    }
    std::fprintf(f, "</vtkOverlappingAMR>\n</VTKFile>\n");
    std::fclose(f);
}
